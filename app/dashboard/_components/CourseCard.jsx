...
function CourseCard({ course, refreshData, displayUser = false }) {
    ...
    try {
      if (course?.courseBanner != "/placeholder.png") {
        const filePath = course?.courseBanner
          .replace(
            "https://firebasestorage.googleapis.com/v0/b/NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET/o/",
            ""
          )
          .split("?")[0];
  ...
  return (
    <div className="shadow-sm rounded-lg border p-2 hover:scale-105 transition-all cursor-pointer mt-4">
      <Link
        href={
          course?.publish
            ...
            : `/create-course-path/${course?.courseId}`
        }
      >
        <Image
      ...
      {!displayUser && course?.publish == false && (
        <Link href={`/create-course-path/${course?.courseId}`}>
